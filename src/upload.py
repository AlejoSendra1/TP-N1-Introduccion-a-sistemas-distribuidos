...
def setup_argparse():
    ...
    parser.add_argument(
        "-H", "--host", type=str, default="localhost", help="server IP address"
    )
    parser.add_argument(
    ...
    # file configuration
    parser.add_argument(
        "-s", "--src", type=str, required=True, help="source file path"
    )
    parser.add_argument(
        "-n",
        "--name",
...
def main():
    ...
    try:
        ...
        sender = create_sender(
            protocol, clientSocket, (args.host, args.port), logger
        )

        # perform handshake first
        if not sender.perform_handshake(
            args.name, os.path.getsize(args.src), protocol
        ):
            return False
