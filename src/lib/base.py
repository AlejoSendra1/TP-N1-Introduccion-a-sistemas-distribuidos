...
)  # buffer size for DATA packets (Selective Repeat)
SW_DATA_BUFFER_SIZE = (
    HEADER_SIZE + SW_PACKET_SIZE
)  # buffer size for DATA packets (Stop & Wait)
...
class RDTPacket:
    ...
    def __init__(
        ...
        self.ack_num = (
            ack_num % SEQ_NUM_MODULO
        )  # acknowledgment number (0-255)
    ...
    def to_bytes(self) -> bytes:
        ...
        if (
            self.packet_type == PacketType.UPLOAD_INIT
            or self.packet_type == PacketType.DOWNLOAD_INIT
        ):
        ...
        if (
            ...
            try:
                session_id_byte = (
                    int(self.session_id) if self.session_id else 0
                )
    ...
    def from_bytes(cls, data: bytes) -> "RDTPacket":
        ...
        if len(data) < HEADER_SIZE:
            raise ValueError(
                f"Invalid packet: too short (expected {HEADER_SIZE},"
                f" got {len(data)})"
            )
        ...
        if (
            ...
            or packet_type == PacketType.DOWNLOAD_INIT
        ):
...
def wait_for_init_packet(
    sock: socket.socket, timeout: Optional[float] = None
) -> Optional[Tuple[RDTPacket, Tuple[str, int]]]:
    """
    Wait for and receive an INIT packet (UPLOAD_INIT or
    DOWNLOAD_INIT) from any client
    ...
    try:
        ...
        if packet.packet_type in (
            PacketType.UPLOAD_INIT,
            PacketType.DOWNLOAD_INIT,
        ):
            return packet, addr
        else:
...
class AbstractSender(ABC):
    ...
    def __init__(
        self, socket: socket.socket, dest_addr: Tuple[str, int], logger
    ):
        self.socket = socket
    ...
    def send_file(self, filepath: str, filename: str) -> bool:
        ...
        try:
            # validate file (basic, maybe should be more
            # strict or throw an exception to handle)
            ...
            self.logger.info(
                f"Starting file transfer: {filename} ({len(packets)} packets)"
            )
    ...
    def _prepare_packets(self, filepath: str) -> List[RDTPacket]:
        ...
        with open(filepath, "rb") as file:
            ...
            while True:
                ...
                packet = RDTPacket(
                    seq_num=chunk_index
                    % SEQ_NUM_MODULO,  # wrap around at SEQ_NUM_MODULO
    ...
    def perform_handshake(
        self, filename: str, file_size: int, protocol: Protocol
    ) -> bool:
                    ...
                    if accept_packet.data:
                        try:
                            ...
                            if self._reconnect_to_dedicated_port(
                                ...
                                self.socket.sendto(
                                    ack_packet.to_bytes(), self.dest_addr
                                )
                                self.logger.info(
                                    f"Sent ACCEPT_ACK - Handshake complete, "
...
class AbstractReceiver(ABC):
    ...
    def receive_file(
        ...
        try:
            ...
            # validate source (only check host, not port - OS may
            # assign different port when client is reconnecting)
            if addr[0] != client_addr[0]:
                self.logger.error(
                    f"Packet from unexpected host: {addr[0]} "
                    f"(expected {client_addr[0]})"
                )
                return False, b""
            ...
            if first_packet.session_id != session_id:
                self.logger.error(
                    f"Invalid session ID: {first_packet.session_id}"
            ...
            self.logger.error(
                f"Error receiving first packet in session {session_id}: {e}"
            )
            return False, b""
    ...
    def _handle_fin(
        self, fin_packet: RDTPacket, addr: Tuple[str, int]
    ) -> bool:
        last_fin_time = time.time()
        self.logger.debug(
            f"Handling FIN packet for session {fin_packet.session_id}"
        )
        fin_ack = RDTPacket(
            packet_type=PacketType.FIN_ACK,
            session_id=fin_packet.session_id
            if hasattr(fin_packet, "session_id") and fin_packet.session_id
            else "",
        )
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                if (
                    ...
                    and fin_packet.session_id == packet.session_id
                    and addr == rcv_addr
                ):
                    # send FIN ACK
                    self.socket.sendto(fin_ack.to_bytes(), addr)
                    self.logger.info(
                        f"Session {fin_packet.session_id} resending FIN ACK"
                    )
                    retries += 1
                    last_fin_time = time.time()
                    ...
                    self.logger.warning(
                        f"Invalid FIN packet from {addr}, "
                        f"expected: {PacketType.FIN},"
                        f"{fin_packet.session_id},{addr}  "
                        f"received: {packet.packet_type},"
                        f"{packet.session_id},{rcv_addr}"
                    )
                    if time.time() - last_fin_time > DUPLICATED_FIN_TIMEOUT:
                        self.logger.debug(
                            "No duplicated FIN received before "
                            "timeout, finishing session"
                        )
                        return True
                ...
                return False
        self.logger.warning(
            "Fin retries limit reached, forcibly ending the session"
        )
        return False
