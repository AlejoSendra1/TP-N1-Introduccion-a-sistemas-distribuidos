...
class RDTSender(AbstractSender):
    ...
    def _prepare_packets(self, filepath: str) -> List[RDTPacket]:
        """Prepare DATA packets from file
        using SW_PACKET_SIZE for Stop & Wait"""
        packets = []
        ...
        with open(filepath, "rb") as file:
            ...
            while True:
                ...
                packet = RDTPacket(
                    seq_num=chunk_index
                    % 2,  # alternating 0, 1 for Stop & Wait
    ...
    def _send_packet_reliable(self, packet: RDTPacket) -> bool:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                ...
                self.logger.debug(
                    f"Sent packet {packet.seq_num} (attempt {attempt + 1})"
                )
                ...
                if (
                    ...
                    and ack_packet.ack_num == packet.seq_num
                    and ack_packet.verify_checksum()
                ):
                    self.logger.debug(
                        f"Received ACK for packet {packet.seq_num}"
                    )
                    return True
                else:
                    self.logger.debug(
                        f"Invalid ACK for packet {packet.seq_num}, retrying..."
                    )

            except timeout:
                self.logger.debug(
                    f"Timeout for packet {packet.seq_num}, retrying..."
                )
            except Exception as e:
                self.logger.error(
                    f"Error sending packet {packet.seq_num}: {e}"
                )
                return False

        self.logger.error(
            f"Failed to send packet {packet.seq_num} after "
            f"{MAX_RETRIES} attempts in session {self.session_id}"
        )
        return False
...
class RDTReceiver(AbstractReceiver):
    ...
    def receive_file_with_first_packet(
        ...
        self.logger.debug(
            f"Verifying first packet with seq {first_packet.seq_num}"
        )
        if not first_packet.verify_checksum():
            self.logger.error("First packet has invalid checksum")
            return False, b""

        self.logger.debug("Checking sequence number of first packet")
        if first_packet.seq_num != self.expected_seq:
            self.logger.warning(
                f"Unexpected sequence number {first_packet.seq_num}"
        ...
        ack = RDTPacket(
            seq_num=0,
            packet_type=PacketType.ACK,
            ack_num=first_packet.seq_num,
            session_id=first_packet.session_id
            if hasattr(first_packet, "session_id") and first_packet.session_id
            else "",
        )

        self.logger.debug("Sending ACK for first packet")
        self.socket.sendto(ack.to_bytes(), addr)
    ...
    def _continue_receiving(
        self, addr: Tuple[str, int], data_queue: queue.Queue
    ) -> Tuple[bool, bytes]:
        """Continue receiving remaining packets"""
        file_data = b""
            ...
            try:
                ...
                if packet_addr[0] != addr[0]:
                    self.logger.warning(
                        ...
                        f"{packet_addr[0]} (expected {addr[0]})"
                    )
                    continue
                ...
                if not packet.verify_checksum():
                    self.logger.error(
                        f"Packet {packet.seq_num} has invalid checksum"
                    )
                    continue
                ...
                if packet.seq_num == self.expected_seq:
                    ...
                    # Put the entire data chunk in
                    # the queue instead of individual bytes
                    ...
                    self.socket.sendto(ack.to_bytes(), addr)
                    self.logger.debug(
                        f"Sent ACK for packet {packet.seq_num}"
                        f" to address: {addr}"
                    )
                    ...
                    self.logger.debug(
                        f"Duplicate packet {packet.seq_num}, "
                        f"expected {self.expected_seq}"
                    )
                    # send ACK for the received packet
                    # (duplicate ACK with session_id if present)
                    ack = RDTPacket(
                        seq_num=0,
                        packet_type=PacketType.ACK,
                        ack_num=packet.seq_num,
    ...
    def receive_file_after_handshake(
        ...
        )  # TODO: do not separate first
        # packet reception from the rest of the file !!!!
