...
def setup_argparse():
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=49153, help="server port"
    )
    ...
    parser.add_argument(
        "-n", "--name", type=str, required=True, help="file name to download"
    )
...
def receive_downloaded_file(
    receiver, logger, data_queue: queue.Queue
) -> Tuple[bool, bytes]:
    """
    ...
    try:
        ...
        with open(filepath, "wb") as f:
            while True:
                ...
                f.write(
                    byte_chunk
                )  # Now byte_chunk is already bytes, not individual bytes
...
def main():
    ...
    if os.path.exists(args.dst):
        response = input(
            f"File '{args.dst}' already exists. Overwrite? (y/N): "
        )
        if response.lower() != "y":
    ...
    signal.signal(
        signal.SIGINT, lambda s, f: signal_handler(s, f, logger, clientSocket)
    )
    signal.signal(
        signal.SIGTERM, lambda s, f: signal_handler(s, f, logger, clientSocket)
    )
    ...
    try:
        ...
        thread_writer = threading.Thread(
            target=write_to_file, args=(data_queue, args.dst, logger)
        )
        thread_writer.start()
        success, file_data = receive_downloaded_file(
            receiver, logger, data_queue
        )
        ...
        try:
            ...
            thread_writer.join()
            logger.debug(
                f"Thread writer joined, checking for file: {args.dst}"
            )
            ...
            if os.path.exists(args.dst):
                ...
                logger.info(
                    f"File saved successfully: {args.dst} ({file_size} bytes)"
                )
            else:
                logger.error(
                    f"Thread writer did not create the expected file:"
                    f" {args.dst}"
                )
